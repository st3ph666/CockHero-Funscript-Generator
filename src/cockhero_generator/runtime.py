"""Compatibility loader for the CockHero v4.15.1 standalone application.
...
def load_standalone() -> ModuleType:
    ...
    script = root / "CockHero_Funscript_Generator_v4.15.1-English-Code-Comments.py"
    if not script.is_file():
        raise FileNotFoundError(f"CockHero standalone script not found: {script}")

    spec = importlib.util.spec_from_file_location("cockhero_generator._standalone_v4_15_1", script)
